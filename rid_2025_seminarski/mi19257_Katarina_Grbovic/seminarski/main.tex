...
\section{Uvod}
% Skoro svaki aspekt svakodnevnog života današnjice je nemoguće zamisliti bez savremene tehnologije. Uz pomoć računara, telefona i interneta se dopisujemo, plaćamo račune, učimo, obavljamo kupovinu i teško nam je da zamislimo da je moguće nauditi našim telefonima. 

U današnje doba, praktično čitav život savremenog čoveka nalazi se na njegovom telefonu i na internetu. Svaki dan listamo socijalne mreže, obavljamo kupovinu i slušamo muziku, a naše podatke držimo u \textit{klaudu} \cite{klaud}. Skoro nam je nezamislivo da neko lako može da naruši bezbednost naših uređaja, kao i da neovlašćeno pristupi našim podacima koje ostavljamo na internetu. \\
Često kada pomislimo na bezbednost našeg računara ili bezbednost interneta pomislimo na kompjuterske viruse, spam mejlove i hakere - maliciozne napadače koji žele da upadnu u naš sistem. Ovakvi vidovi napada su sada iza nas, međutim hakeri i druge zlonamerne strane su se prilagodile promenama i savremene metode napada su sofisticiranije, opasnije i razornije nego pre. 
Kako bismo zaštitili sisteme uređaja i mreža od ovakvih napada, razvila se grana informacione bezbednosti (eng. \textit{cybersecurity}) \cite{cybersec}. \\ 
Informaciona bezbednost se fokusira na zaštitu mreža, aplikacija i informacija od neautorizovanog pristupa ili napada. Međutim, delotvorna odbrana se ne ogleda samo u njenim mehanizmima, već i u razumevanju kako ti mehanizmi odbrane mogu da zakažu - ovim zadatkom se bavi podoblast informacione bezbednosti - ofanzivna bezbednost. 
\\
\\
\textbf{Ofanzivna bezbednost}  (eng. \textit{Offensive security} - \textbf{OffSec}) je grana informacione bezbednosti koja menja način na koji pristupamo zaštiti sistema i računarskih mreža - umesto da se brani sistem od napadača, ideja je da se pretvaramo da smo napadač, ali sa jasno postavljenim legalnim i etičkim granicama. Svrha ofanzivne bezbednosti je da uoči slabosti u sistemima pre samih napadača kroz testiranje i simulaciju realnih napada. Njih izvršavavaju \textit{etički hakeri} 
\cite{ibm_offensive_security}. \\

U okviru ofanzivne bezbednosti, \textit{red team} \cite{redblue} predstavlja grupu stručnjaka čiji je cilj sveobuhvatno testiranje i simulirani napadi na sisteme kompanije i njenih zaposlenih, kako bi se utvrdilo na koji način kompanija može da uoči, odgovori i oporavi se od stvarnih napada.
\\

\textbf{Defanzivna bezbednost} (eng. \textit{Defensive security}) takođe predstavlja niz bezbednosnih praksi i rešenja koja za cilj imaju zaštitu sistema, mreža i podataka od sajber pretnji i napada kroz prevenciju, uočavanje i odgovor. \\
U odnosu na crveni tim, \textit{blue team} \cite{redblue} čini grupa eksperata čiji je osnovni zadatak da štite sistem kompanije od spoljašnjih napada.\\

% \subsection{Razlika između ofanzivne i defanzivne bezbednosti}

Defanzivna bezbednost gleda na napadača kao na protivnika od koga se treba odbraniti, dok ofanzivna bezbednost gleda kako može da iskoristi napadačeve taktike i primeni ih u svojim rešenjima \cite{defensive_security}.


\subsection{Pentesting}
Unutar oblasti ofanzivne bezbednosti, jedna od najprihvaćenijih metoda rada jeste upravo penetraciono testiranje  \textit{(eng. Penetration Testing - pentesting)}. Zadaci ofanzivne bezbednosti podrazumevaju širok spektar tehnika za otkrivanje ranjivosti određenih sistema, dok se pentesting fokusira specifično na simulaciju pravih sajbernapada na sisteme, mreže ili aplikacije. Drugim rečima, ofanzivna bezbednost generalno predstavlja pristup "razmišljaj kao napadač", dok je pentest struktuirana, kontrolisana metoda upravo tog pristupa.
Pentesting se radi ručno, i samim tim predstavlja sveobuhvatniji proces pronalaženja bezbednosnih propusta.
...
Penetraciono testiranje dolazi u više oblika u zavisnosti od toga \textit{šta} se testira i \textit{kako} se test izvršava. Testovi mogu da targetiraju mreže, veb aplikacije, klaud okruženja, uređaje ili čak ljude - i da prate ustanovljene metodologije koje daju strukturu samog testa, odnosno korake i pravila po kojima se test izvršava.
...
\begin{itemize}
  \item \textbf{Mrežni pentesting} - testira se čitava mrežna infrastruktura na potencijalni napad \cite{ibm_offensive_security}. \\
  \item \textbf{Pentesting bežičnih mreža} - ovo je testiranje WAN-a i protokola za bežičnu komunikaciju. \\
  \item \textbf{Pentesting web aplikacija} - otkrivaju se greške u kodu i dizajnu kod web sajtova i aplikacija povezanih na internet. \\ 
  \item \textbf{Pentesting mobilnih aplikacija} - testiranje i proučavanje različitih komponenti mobilne aplikacije koje mogu otkriti potencijalne bezbednosne propuste u izvornom kodu same mobilne aplikacije.
   \item \textbf{Build i konfiguracija sistema} - služi za utvrđivanje propusta u konfiguraciji kroz veb servere, aplikacione servere, rutere i fajrvol (\textit{eng. firewall}) \cite{firewall}. \\
  \item \textbf{Socijalni inžinjering} - testeri pokušavaju da od zaposlenih posredno izvuku osetljive informacije, da kliknu na zlonamerni link ili da im daju pristup sistemu, bez da eksplicitno pre toga hakuju sistem. \\
...
\begin{itemize}
\item \textbf{Port skeneri} - otkrivaju otvorene portove unutar nekog sistema. Otvoreni portovi sugerišu koji vektori napada se dalje mogu koristiti na tom sistemu kao i neke dodatne informacije o sistemu.
  \item \textbf{Skeniranje ranjivosti} - traže se poznate slabosti sistema, kao i loša konfiguracija koja može biti zloupotrebljena od strane napadača. 
  \item \textbf{Alati za prisluškivanje mrežnog saobraćaja} - alati koji slušaju mrežni saobraćaj. Korisno prilikom identifikacije da li su podaci u saobraćaju enkriptovani, kao i da se identifikuju potencijalno nebezbedne putanje komunikacije.
  \item \textbf{Veb proksiji} - testeri uz pomoć veb proksija mogu uticati i menjati saobraćaj između pretraživača i veb servera kompanije. 
  \item \textbf{Pogađanje šifara} - napadom grube sile korisničkih šifara unutar sistema je jedan od najčešćih sajber napada. Omogućava testeru da zaključi da li zaposleni unutar kompanije koriste slabe ili jake lozinke \cite{pentesting-tools}. 
\end{itemize}

\subsection{Eskalacija privilegija}

Eskalacija privilegija (eng. \textit{Privilege esalation}) predstavlja dobijanje neautorizovanog pristupa nad osetljivim resursima određenog sistema zloupotrebljavajući bagove, lošu konfiguaciju operativnog sistema ili softvera. U pentestingu, eskalacija privilegija je važna jer demonstrira koliki je stvaran uticaj ranjivosti određenog sistema - na primer ukoliko napadač pronađe korisnika i uspe da dobije prava administratora, može čitati sve fajlove, instalirati zlonameran softver i može imati kontrolu nad čitavim sistemom. 

Eskalacija privilegija je u pentestingu potrebna iz više razloga. Zahvaljujući eskalaciji privilegija možemo proceniti koliko štete napad može da napravi ukoliko napadač zloupotrebi određenu ranjivost. Možemo proceniti i \textit{lance napada} - kompromitovanje često počinje od sitne ranjivosti gde se kroz eskalaciju dolazi do ključnih resursa. Na kraju, eskalacija privilegija nam daje konkretne dokaze o ranjivosti i korake za popravku ranjivog sistema.
...
\begin{itemize}
  \item \textbf{Horizontalna eskalacija privilegija} - napadač siri svoje privilegije tako što preuzima drugi nalog i preuzima legitimne privilegije dodeljene tom korisniku.
    \item \textbf{Vertikalna eskalacija privilegija} - napadač pokušava da preuzme druge privilegije sa nalogom na kom se već nalazi. Npr. napadač se nalazi na regularnom nalogu korisnika, ali pokušava da dobije privilegije administratora ili \textit{root} pristup \cite{privilege-escalation}.
\end{itemize}
...
Penetraciono testiranje simulira ofanzivne akcije koje bi u suprotnom bile nelegalne ukoliko bi se radile bez dozvole. Legalni ugovori i etička pravila postoje kako se zaštitio tester, klijent i treća strana. Ovime se takođe osigurava da testiranje pronalazi ranjivosti bez činjenja prave štete i da ispunjava regulative koje zahtevaju rednovno bezbednosno testiranje. 
...
Sa rastom i razvojem tehnologije, ofanzivna bezbednost takođe evoluira. Klasično penetraciono testiranje više nije dovoljno jer su današnji napadi kombinovani, automatizovani i veoma često koriste veštačku inteligenciju. 
\newline
Zbog ovoga, pojavljuju se novi koncepti poput \textit{purple teaming}-a i \textit{AI pentesting}-a koji spajaju ofanzivne i defanzivne pristupe i koriste automatizaciju za bržu i dublju detekciju ranjivosti. Cilj nije više samo pronaći slabosti, već neprestano testirati, učiti i unapređivati bezbednosne mehanizme u realnom vremenu.  
\subsection{Purple teaming}
Purple team je grupa sajber bezbednosnih profesionalaca koji simuliraju maliciozne napade i pentesting da bi identifikovali ranjivosti i predložili strategije za organizacije u IT infrastrukturi. Izraz je izveden od reci ljubičasto što simbolizuje kombinaciju crvenog i plavog tima. Za razliku od tradicionalnog crvenog i plavog tima koji su obično podeljene uloge, ljubičasti tim radi u bliskoj koordinaciji i saradnji, deli informacije i uvid kako bi uočili ranjivosti i unapredili sveukupno stanje bezbednosti sistema.\\
Ljubičasti tim nudi isti benefit kao i crveni i plavi tim. On omogućava organizacijama da aktivno testiraju postojeće ranjivosti u njihovim sistemima. Ujedinjenje crvenog/plavog tima omogućava organizaciji da identifikuje lošu konfiguraciju i pokriva rupe u postojećim bezbednosnim proizvodima, ojačava mrežnu bezbednost i podiže upućenost osoblja u rizike ljudske ranjivosti koji mogu da kompromituju bezbednost organizacije. 
Ujedinjenje crvenog i plavog tima kao jednu jedinicu nudi dodatne benefite kao što je povećanje zaštite kroz konstantan fibek i znanje podeljeno izmedju ujedinjenog ofanzivnog i defanzivnog tima. Konstantno testiranje preneseno je kroz konstantno angažovanje ljubičastog tima. \cite{purple-teaming}.
\subsection{AI u pentesting napadima i odbrani}
AI pentestiranje obezbeđuje bezbednost veštacke inteligencije sistema tako što simulira napade koji eksploatišu ranjivosti AI modela. Za razliku od tradicionalnog pentestinga koji se fokusira na mrežu, aplikacije i infrastrukturu, AI pentesting otkriva slabosti u AI modelima arhitekture, rada sa podacima i procesom donošenja odluke \cite{ai-pentesting}.
\begin{itemize}
  \item \textbf{Trovanje podataka} - u ovoj vrsti napada, haker ubacuje zlonamerne podatke u kolekcije podataka za treniranje, korumpirajući model za proces učenja. Kao rezultat, AI sistem prihvata netačne šablone i ponašanja, kompromitujući pouzdanost njegovih izlaza. Validacija podataka i anomalije detektovane pomažu zaštiti treniranih podataka da identifikuju i filtriraju sumnjive ili izmanipulisane ulaze \cite{ai-pentesting}.
