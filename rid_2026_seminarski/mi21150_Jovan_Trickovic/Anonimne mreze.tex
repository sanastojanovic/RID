...
\protect\hyperlink{_Toc234141038}{\textbf{1. Uvod}
\protect\hyperlink{_Toc234141038}{3}}

\protect\hyperlink{kako-funkcioniux161u-vpn-i-tor}{\textbf{2. Kako
funkcionišu VPN i Tor}
\protect\hyperlink{kako-funkcioniux161u-vpn-i-tor}{3}}

\protect\hyperlink{_Toc234141040}{\textbf{2.1 VPN}
\protect\hyperlink{_Toc234141040}{3}}

\protect\hyperlink{_Toc234141041}{\textbf{2.2 Tor mreža}
\protect\hyperlink{_Toc234141041}{4}}

\protect\hyperlink{_Toc234141042}{\textbf{2.3 Onion adrese}
\protect\hyperlink{_Toc234141042}{5}}
...
\protect\hyperlink{_Toc234141044}{\textbf{3.1 Operation security OpSec}
\protect\hyperlink{_Toc234141044}{5}}

\protect\hyperlink{_Toc234141045}{\textbf{3.2 Kako se tor brani od
fingerprinting a} \protect\hyperlink{_Toc234141045}{5}}

\protect\hyperlink{_Toc234141046}{\textbf{3.3 Deep packet inspection i
metode cenzure} \protect\hyperlink{_Toc234141046}{7}}

\protect\hyperlink{_Toc234141047}{\textbf{3.4 Kombinovanje tor mreže i
VPN usluga} \protect\hyperlink{_Toc234141047}{7}}

\protect\hyperlink{_Toc234141048}{\textbf{3.5 Decentralizovani VPN i
alternativne mreže} \protect\hyperlink{_Toc234141048}{8}}
...
\protect\hyperlink{_Toc234141050}{\textbf{5. Veza sa temama kursa}
\protect\hyperlink{_Toc234141050}{10}}

\protect\hyperlink{_Toc234141051}{\textbf{6. Ključni radovi}
\protect\hyperlink{_Toc234141051}{11}}

\protect\hyperlink{_Toc234141052}{\textbf{7. Zaključak}
\protect\hyperlink{_Toc234141052}{11}}

\textbf{\hfill\break
}

\protect\hypertarget{_Toc234141038}{}{}\textbf{1. Uvod}
...
``Pew Research Center'' ukazuje i na to. 56\% uvek pritisne ``Slažem
...
\protect\hypertarget{_Toc234141040}{}{}\textbf{2.1 VPN}
...
hoće da se zaštiti, jer jedan alat ne može da pokrije sve potrebe. Jedan
od načina da se korisnik zaštiti na nivou IP adresa je VPN. Korisnik
uspostavlja enkriptovanu \emph{point-to-point} konekciju sa serverom.
Praktično, zamenjuje svoju IP adresu koju mu je dodelio \emph{ISP}
(Internet Service Provider) sa adresom koju mu je dodelio VPN server.
\emph{ISP} samo vidi da je korisnik uspostavio vezu sa VPN-om, ali ne
...
vera u njega bitna. Ipak postoje 3rd-party kompanije koje vrše revizije
(audits) i javno objavljuju svoje rezultate, što služi kao ``dokaz'' da
...
pravnim pritiskom da prikuplja podatke. Zato je bitno da VPN provajderi
budu registrovani u državama sa jačim zakonima o privatnosti.

\textbf{Curenje DNS-a (DNS leakage)} Kada korisnik pretražuje internet,
...
\protect\hypertarget{_Toc234141041}{}{}\textbf{2.2 Tor mreža}
...
3 između sebe i destinacije, gde svaki čvor zna prethodnika i
...
zahtev. Ovo se uspostavlja korišćenjem PFS (Perfect Forward Secrecy),
...
(entry guard) koristeći ``Diffie-Hellman rukovanje''. Diffie-Hellman
ukratko radi sledeće: klijent i server biraju javne brojeve g
(generator) i p (veliki prost broj). Onda svaki bira svoj privatni ključ
(a i b). Onda klijent izračuna svoj javni ključ \(A = g^{a}\ mod\ (p)\)
(analogno server izračuna B). Na kraju, klijent izračuna
\(s = \ B^{a}\ mod\ (p)\) (server koristi A i b umesto B i a). Ovo ``s''
se koristi kao ključ enkripcije. Nakon što je ta veza uspostavljena,
onda proširuje zahtev na \emph{middle} čvor i na kraju šalje zahtev ka
\emph{exit} čvoru. Pošto svaki sloj ima svoj ključ koji nestaje nakon
što se konekcija ugasi, ovo čini da u slučaju da neki od čvorova bude
kompromitovan kasnije, nije moguće dekriptovati prethodni internet
saobraćaj. Po ovom modelu slojevanja je Tor i dobio svoje ime ``The
Onion Router'', gde svaki sloj štiti prethodni, kao kod luka {[}12{]}.
Ipak, Tor mreža je sporija od VPN-a i sajtovi cešće blokiraju ip adrese
čvorova nego VPN ip adrese, pa to treba isto uzeti u obzir.
...
tehnologiju koja bi omogućavala da srednji čvor funkcioniše kao krajnji
čvor i time procesuira više multipleksovanih TCP protoka, ali se to u
praksi trenutno ne koristi {[}12{]}. Problem kod Tor mreže je to šta ako
neko ima pristup početnom i krajnjem čvoru? U tom slučaju je moguće
pratiti saobraćaj kroz početni i kroz krajnji čvor. Pošto prvi čvor zna
klijentovu IP adresu, i onaj koji posmatra \emph{entry guard} je isto
zna. Uz to zna odredišnu IP adresu. Korišćenjem ML modela i statistike,
moguće je uz pomoć metapodataka, saobraćaja, dužina prenosa itd. da se
koreliše korisnik sa tim saobraćajem. Ovo se zove \emph{end-to-end
traffic confirmation attack} i nije ga moguće potpuno zaobići, ali ideja
je da se smanji rizik tako što se smanjuje skup entry guard-ova {[}8,
12{]}. Srednji i krajnji čvorovi se mnogo češće smenjuju, ali to nije
slučaj sa početnim čvorom. Pošto svako može da hostuje Tor čvor, to
znači da maliciozni učesnik može da bude deo mreže. Smanjenjem početnog
skupa se ta šansa smanjuje, jer je onda i manja šansa da naiđemo na
maliciozni čvor. Sledeći problem kod Tor mreže je \emph{exit node
trust}. Svaki čvor vrši enkripciju, ali krajnji čvor mora da dekriptuje
poruku kako bi je isporučio odredišnoj IP adresi. Opet se vraćamo na
problem volontera. Veliki dobitak koji Tor mreža ima u poređenju sa
VPN-om je to što se ne mora verovati centralizovanom entitetu, ali je to
u isto vreme i nedostatak. Ako saobraćaj između klijenta i odredišta ne
koristi enkriptovani protokol kao HTTPS, i umesto toga koristi HTTP, i
ako je čvor maliciozan, moguće je logovati sav saobraćaj. Takođe je
moguće izvršiti tehniku ``\emph{SSL stripping}'', gde napadač degradira
(downgrade-uje) HTTPS konekciju na HTTP konekciju i time može da čita
saobraćaj {[}8{]}.

\protect\hypertarget{_Toc234141042}{}{}\textbf{2.3 Onion adrese}
...
korišćenja Tor mreže i na deep web koji koristi onion adrese i kojima se
...
\item
  ...
  u decentralizovanu heš tabelu (\emph{DHT}) {[}12{]}.
\item
  Kada klijent želi da se konektuje na onion server, on čita \emph{DHT}
  i uzima jedan od introduction points kome šalje enkriptovan zahtev i
  takođe uzima neki drugi čvor za sebe kao rendezvous point {[}12{]}.
\item
...
\protect\hypertarget{_Toc234141044}{}{}\textbf{3.1 Operation security
OpSec}
...
na clear web-u. Idealno bi bilo ne mešati naloge koji se koriste sa
regularnim pretraživačem sa onima koji se koriste tokom koršćenja Tor
...
da se korisnik fingerprint-uje.

\protect\hypertarget{_Toc234141045}{}{}\textbf{3.2 Kako se tor brani od
fingerprinting a}
...
\item
  ...
  prisilno postavlja na \emph{UTC} (Coordinated Universal Time), bez
  obzira na geografsku lokaciju korisnika.
\end{itemize}

\protect\hypertarget{_Toc234141046}{}{}\textbf{3.3 Deep packet
inspection i metode cenzure}

Iako VPN i Tor pružaju snažnu enkripciju, njihovo korišćenje nije
nevidljivo. U odredjenim državama, internet provajderi (\emph{ISP}) i
državni cenzori koriste tehniku poznatu kao dubinska analiza paketa
(Deep Packet Inspection - \emph{DPI}). Standardna analiza paketa gleda
samo "zaglavlje" (header) kako bi saznala izvor i destinaciju
saobraćaja, dok \emph{DPI} dodatno analizira samu frekvenciju i
metapodatke sadržaja koji se prenosi. Na ovaj način, softver može da
prepozna "potpis" (signature) OpenVPN protokola, WireGuard-a ili
...
\protect\hypertarget{_Toc234141047}{}{}\textbf{3.4 Kombinovanje tor
mreže i VPN usluga}
...
\protect\hypertarget{_Toc234141048}{}{}\textbf{3.5 Decentralizovani VPN
...
Pored Tor-a, postoje i druge ``dark web'' mreže koje pristupaju
privatnosti iz drugog ugla, poput \emph{I2P} (Invisible Internet
Project). \emph{I2P} je izgrađen isključivo kao zatvoreni ekosistem za
peer-to-peer (P2P) komunikaciju unutar same mreže, tj. ne može se
koristiti za noramalan surface web internet. Pošto I2P ne koristi
...
\protect\hypertarget{_Toc234141050}{}{}\textbf{5. Veza sa temama kursa}

Najveći deo kojim se rad oslanja na teme kursa „Računarstvo i društvo''
je u pordručju privatnosti na internetu. Ceo rad se zasniva na tome kako
koristi VPN i Tor da bi se ostvarila veća privatnost na internetu, pa je
samim tim to i najbitnija veza.\\
Druga tema se oslanja na etiku, gde je spomenuto kako ovi alati imaju
dvostruku namenu, pa time povlače i etička pitanja o tome da li treba
regulisati i koliko ove alate.\\
Etički deo je i usko povezan sa slobodom govora, jer se Tor koristi u
zemljama sa većom kontrolom interneta da bi se izbegao državni nadzor.\\
Još jedna, ali slabija veza od ostalih se može ostvariti sa temom
bezbednosti, jer Tor mreža i VPN koriste kritpografske tehinke da bi
zaštitile podatke tokom prenosa, uključujući i opsec sekciju.

\protect\hypertarget{_Toc234141051}{}{}\textbf{6. Ključni radovi}

\textbf{6.1 Dingledine et al. (2004) ``Tor: The Second-Generation Onion
Router''}\\
\strut \\
Ovaj rad predstavlja srž cele arhitekture Tor mreže. U njemu je opisan
...
njemu su opisani svi koncepti kao što su Diffie Hellman rukovanje i
relay čvorovoa, kao i glavni cilj ove mreže. Takođe su opsiane i druge
...
predstavlja jedan od originalnih modela Tor mreže.\\
\strut \\
\strut \\
...
kroz Tor nije usemerena ka onion adresama, što je često popularno
mišljenje. Mada, iako su onion adrese često povezame sa ilegalnim
aktivonistma, to nije tačno, a ovo dodatno pokazuje da ideja da samo
krimanlci koriste Tor nije tačna.

\protect\hypertarget{_Toc234141052}{}{}\textbf{7. Zaključak}
...
rešenje ima svoje mane i prednosti, pa je zato bitno da korsnik izabere
